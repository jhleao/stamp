...
SCENES = [
    ...
    {
        "eyebrow": "2 · CONNECT YOUR TEAM",
        "title": "Use an ordinary Drive folder",
        "code": "stamp google-oauth client-secret.json\nstamp doctor && stamp login\nstamp space create 'My Team'",
        "body": "A Space is simply a folder in My Drive or a Shared Drive.\nDrive already supplies permissions, history, and recovery.",
        "narration": "Next, connect Google Drive using a desktop OAuth client. Run Stamp doctor to verify the complete setup, sign in, and create a Space. A Space is simply an ordinary folder in My Drive or a Shared Drive. Stamp does not add a server or database. Drive already supplies team permissions, revision history, and recovery.",
    },
    {
        "eyebrow": "3 · MAKE A REUSABLE LOOK",
        "title": "A theme is just a folder",
        "code": "stamp template create my-theme\n# Edit examples and components\nstamp template preview my-theme",
        "body": "HTML frames · Tailwind · local assets · examples\ncomponents/metric-card.tsx -> <metric-card>",
        "narration": "To make your own visual system, create a theme. A theme is just a folder containing HTML frames, a Tailwind design system, local assets, reusable components, and examples. There is no package manager or install step. A component file named metric card becomes a metric card tag that stays visually simple inside Markdown.",
    },
    {
        ...
        "code": "stamp project create board-pack \\\n+  --name 'Board Pack' --template my-theme\ncd board-pack && stamp studio",
        "narration": "Point a coding agent at the theme folder and describe the result you want. Ask it to match your brand, add reusable components, create stress test examples, and inspect every preview. Then create a project from the theme. Stamp snapshots the theme, so later theme changes never surprise existing projects.",
    },
    {
